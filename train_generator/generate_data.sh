...
trdg -l ${LANG} --font_dir ./fonts --font 'Moul-Regular,Battambang-Regular' -k 1 -rk  -c ${TOTAL_IMAGES} -tc '#000000,#AAAAAA' -w ${WORDS_PER_IMAGE} -b 1 -bl 1 -rbl -na 2 -f 32 --dict ./${MODEL}.txt -t 32 --word_split --output_dir ${DATASET_FOLDER}/train_data/images
...
trdg -l ${LANG} --font_dir ./fonts --font 'Moul-Regular,Battambang-Regular' -k 1 -rk -c ${TOTAL_VAL_IMAGES} -tc '#000000,#AAAAAA' -w ${WORDS_PER_IMAGE} -b 1 -bl 1 -rbl -na 2 -f 32 --dict ./${MODEL}.txt -t 32 --word_split --output_dir ${DATASET_FOLDER}/val_data/images
